...
function update_click_intersection () {
    ...
    if (last_flat_intersection_point != undefined) {
...
function add_floor (floor,base_delay , floor_delay = 0, last_floor = false) {
    ...
    window.floor_obj[floor_of_mesh][0].parent.scale.set(floor_scale,floor_scale,floor_scale);
    let floor_key = floor_of_mesh + 5;
...
function destroy_building (current_floor_var , flat = null) {
   ...
   // window.roof.visible = false;
    ...
    if (lock_mouse_rotation_x != true) {
        animate_camera_angle(-0.976);
        add_tween_animation ({
            'animation_obj' :  tween_animations,
            'start' : {z : perspectiveCamera.position.z},
            'target' : {  z : 80  },
            'duration' : 1000,
            'easing' :  TWEEN.Easing.Quintic.In,
            'delay' : 400
        }, function (e) {
            perspectiveCamera.position.z = e.z;
            target_zoom = e.z;
            window.change_plan_animation = true;
            window.building_constructing = true;
        }, function (e) {
            // update_flat_labels();
            window.change_plan_animation = false;
            window.building_constructing = false;
        });
...
function add_appartment_info_in_popup (box) {
   ...
   let facing_string_he = '';
    ...
    var floor = flat.userData.floor + 5;
...
function set_click_point_coords (event) {
    ...
    let position_top = event.clientY - (three_js_offset.top - window.scrollY) + 5;
    let position_left = event.clientX - (three_js_offset.left) + 5;
            ...
            window.floor_obj[current_floor].forEach(function(flat, index){
                ...
                setTimeout(function(){
                    ...
                    if (flat.parent.children[0].name.search('zagluha') == -1) {
                        floor_number = flat.parent.children[0].userData.floor + 5;
                    } else {
                        floor_number = flat.parent.children[1].userData.floor + 5;
                    }
                   ...
                   let inner_html = `
                    ...
                    let p;
                    if (dynamic == true) {
                        p = flat.getWorldPosition(new global_three.Vector3());
                    } else {
                        p = flat.userData.world_position;
                    }
...
function floor_appear_animation () {
    ...
    window.floor_obj[current_floor].forEach(function(flat, index){
        if (flat.name != 'floor_center') {
            ...
            let start =  {
                opacity : 0,
                add_position : add_position + 3000 * index,
                number : 0,
            };
            let target = {
