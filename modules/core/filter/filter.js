...
import { ScrollDrag } from './scroll-n-drag.js';
import {filterSettings} from "../../individual/filter/filter.js";
export function  add_filter (container, img_path = 'img/filter-module/') {
    ...
    `;
    let search_fields = filterSettings.search_fields;
    let filter_controls = filterSettings.filter_controls;
    let sorting_controls = filterSettings.sorting_controls;
    ...
    let filter_module_container = `
        <div class="filter-module-container non-canvas clear">
            ...
            <div class="filter-controls open">
                <div class="filter-controls-wrap"></div>
            </div>
    ...
    let element = document.querySelector('.filter-controls');
    container[0].controlsScroll = new ScrollDrag(element);
    container[0].controlsScroll.init();
    ...
    function add_sorting_html () {
        ...
        container.find('.filter-module-container .filter-controls-wrap').append(`<div class="nfm-sorting-box" id="sort-item-sort-tab">
    ...
    function add_filters_html() {
        ...
        container.find('.filter-module-container .filter-controls-wrap').append(filters_html);
        filter_controls.forEach(function(item){
            if (item.filter_type === 'range') {
        ...
        $('.filter-module-open-btn').click(function(){
            ...
            if (!$(this)[0].lastClickTime || $(this)[0].lastClickTime + 1000 < Date.now()) {

                $(this)[0].lastClickTime = Date.now();
...
/*        $('.number-box').click(function(){
    ...
    function update_flat_cards () {
        ...
        flat_slider[0].onwheel = function(event){
            if ($('.nfm-flat-card').length >  get_card_for_screen_width()) {
                ...
                function on_mouse_up(event) {
                    ...
                    if ( event.changedTouches ) {
                        if ($(window).width() < 1024) {
                            ...
                            let positive_current_left = Math.abs(current_left);
                            let last_direction = flat_slider.attr('data-last-direction');
                            let center_modifier = Math.floor(window.outerWidth / 2);
                            //let clear_current_position = Math.floor((positive_current_left) / card_width);
                            let current_position = Math.floor((positive_current_left + center_modifier) / card_width);
                           // let positions_difference = current_position - clear_current_position;
                            let clear_position = current_position * card_width;
                            let difference =  (positive_current_left + center_modifier)  - clear_position;
                            // console.log(difference);
                            let target_card_position;
                          if (last_direction == '-') {
                                if (difference  < card_width / 2 - 30) {
                                    //set_scroll_on_card(card_id);
                                    target_card_position = current_position - 1;

                                } else {
                                    target_card_position = current_position;
                              }
                            } else {
                                if (difference > card_width / 2 + 30) {
                                    target_card_position = current_position + 1;
                            ...
                            $('.nfm-flat-card').eq(target_card_position).click();
                            let target_scroll_position = ((target_card_position * card_width) - center_modifier + (card_width  / 2)) * -1;

                            var easing = r_animation_type;
                            var delay = 0;
                            var animation = new TWEEN.Tween({left : current_left}).to({left : target_scroll_position}, r_animate_duration);
                            TWEEN.add(animation);
                            animation.delay(delay);
                            animation.onStart(function (e) {
                                animate_scroll_bool = false;
                                set_cards_content (current_left)
                                flat_slider.css(position_side, current_left);
                            });
                            animation.onUpdate(function (e) {
                                flat_slider.css(position_side, e.left);
                                set_cards_content (e.left)
                            ...
                            animation.onComplete(function (e) {
                                setTimeout(function(){
                                    let current_postion = Math.floor(Math.abs( target_scroll_position ) / ($('.filter-module-container .flat-cards .nfm-flat-card').outerWidth() + 32));
                                    flat_slider.attr('data-current-position', current_postion );
                                    flat_slider.attr('data-target-left', target_scroll_position);
                                    if (current_left < target_scroll_position) {
                                        flat_slider.attr('data-last-direction', '-');
                                    } else {
                                        flat_slider.attr('data-last-direction', '+');
                                    }
                                    document.querySelector('.flat-cards-slider').style[position_side] = target_scroll_position + 'px';
                                    animate_scroll_bool = true;
    ...
    }
    {
        $('.controls-tab').click(function(){
            ...
            if (key !== 'more') {
                $('.filter-module-container .filter-controls-wrap > *').removeClass('mobile-open');
